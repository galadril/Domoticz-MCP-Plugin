...
class DomoticzOAuthClient:
    ...
    def make_authenticated_request(self, access_token: str, params: dict):
        ...
        PASSTHROUGH MODE: Uses the provided access_token directly to call Domoticz.
        The token should be issued by Domoticz OAuth for the end user.
        
        Args:
            access_token: OAuth token from the MCP client (issued by Domoticz)
            params: API parameters
        """
        try:
            # PASSTHROUGH MODE: Use the token provided by the client
            ...
            Domoticz.Debug("Using OAuth passthrough mode - client token sent to Domoticz")
            ...
            if r.status_code == 401:
                ...
                return {"error": "OAuth token expired or invalid - user needs to re-authenticate", "status_code": 401}
